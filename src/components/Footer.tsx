...
export default function Footer() {
    return (
        <footer className="py-16 pb-20 border-t" style={{ borderColor: 'color-mix(in srgb, var(--border) 10%, transparent)' }}>
            <div className="max-w-6xl mx-auto px-6">
                <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                    ...
                    <p className="text-[var(--text-muted)] text-sm text-center sm:text-left">
                        © 2025 Sunday Samuel. Crafted with passion.
                    </p>
                    ...
                    <div className="flex gap-4">
                        {[
                            ...
                            <a key={social.label} href="#" aria-label={social.label}
                                className="glass-card w-11 h-11 rounded-full flex items-center justify-center text-[var(--text-muted)] transition-all hover:-translate-y-1 hover:text-[var(--text)]"
                                style={{ background: 'color-mix(in srgb, var(--bg-card) 80%, transparent)' }}
                                onMouseEnter={(e) => {
                                    e.currentTarget.style.background = 'var(--accent)';
                                    e.currentTarget.style.color = 'var(--accent-text)';
                                    e.currentTarget.style.boxShadow = '0 8px 25px color-mix(in srgb, var(--accent) 30%, transparent)';
                                }}
                                onMouseLeave={(e) => {
